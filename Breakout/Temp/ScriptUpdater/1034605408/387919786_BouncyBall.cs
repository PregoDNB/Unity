...
public class BouncyBall : MonoBehaviour
{
    ...
    // Dit fungeert als ons centrale 'brein'
    ...
    public static GameObject ballPrefab_s; // Static referentie naar de prefab

    // --- Lokale referenties (alleen voor de eerste bal) ---
    ...
    public GameObject ballPrefab; // De prefab van de bal

    void Awake()
    {
        // De eerste bal die in de scene is, stelt alle gedeelde (static) referenties in.
        if (scoreTxt_s == null)
        {
            scoreTxt_s = scoreTxt;
    ...
    void Start()
    {
        ...
        // Als dit de allereerste bal in een nieuw spel is, reset de game state.
        ...
        // Geef de bal een start-impuls als deze stilstaat
    ...
    void Update()
    {
        // Als een bal van het scherm valt, vernietig hem dan. De OnDestroy functie regelt de rest.
    ...
    void OnDestroy()
    {
        // Als dit de laatste bal in het spel was...
        if (FindObjectsByType<BouncyBall>(FindObjectsSortMode.None).Length <= 1)
        {
    ...
    void LoseLife()
    {
        ...
        {
            // Maak een nieuwe bal aan in het midden van het veld via de prefab.
    ...
    {
        // Maakt een extra bal op de huidige positie.
        if (ballPrefab_s != null)
        {
